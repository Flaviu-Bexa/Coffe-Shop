...
namespace CoffeeShop
{
    public partial class DeliveryForm : Form
    {
        ...
        private void btnShowOrder_Click(object sender, EventArgs e)
        {
            ...
            Globals.currentOrder.address = row.Cells[2].Value.ToString();

            if (row.Cells[3].Value.ToString() == statusOrderTypes.realizare.ToString())
            {
                Globals.currentOrder.status = statusOrderTypes.realizare;
            }
            else if (row.Cells[3].Value.ToString() == statusOrderTypes.platita.ToString())
            {
                Globals.currentOrder.status = statusOrderTypes.platita;
            }
            else if (row.Cells[3].Value.ToString() == statusOrderTypes.curs_de_livrare.ToString())
            {
                Globals.currentOrder.status = statusOrderTypes.curs_de_livrare;
            }
            else if (row.Cells[3].Value.ToString() == statusOrderTypes.asteptare.ToString())
            {
                Globals.currentOrder.status = statusOrderTypes.asteptare;
            }
        ...
        private void DeliveryForm_Load(object sender, EventArgs e)
        {
            ...
            tableOrders.Columns.Add("itemNames", "Produse");
            tableOrders.Columns.Add("orderAddress", "Adresa");
            tableOrders.Columns.Add("orderStatus", "Status");
            ...
            {
                object[] orderDetails = new object[4];
                ...
                foreach (OrderItem orderItem in userOrderItems)
                {
                    if (orderItem.order_id == order.id)
                    {
                        ...
                        orderDetails[2] = order.address;
                        orderDetails[3] = order.status.ToString();
